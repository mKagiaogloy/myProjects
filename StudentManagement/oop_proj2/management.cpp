...
void Management::display_students()
{
	...
	for (auto& student : this->students)
	{	
		std::cout<< index << " " << student << std::endl;
		index++;
	}
}
...
int Management::select_student() {
	...
	do {
		if (error) {
			cout << "Dialekse swsta re bobo"<<endl;
		}
		cin >> student_selected;
		error = true;
	} while (this->students.size() <= student_selected);
...
void Management::update_student() {
	...
	string sid, sfullname;
	int ssemester, scredits;
	this->display_students();
	selected_student = this->select_student();

	cout << "Δώστε νέο ID: ";
	cin >> sid;


	cout << "Δώστε νέο όνομα φοιτητή: ";
	cin.ignore();
	getline(cin, sfullname);


	cout << "Δώστε νέο εξάμηνο φοιτητή: ";
	cin >> ssemester;

	cout << "Δώστε νέες διδακτικές μονάδες φοιτητή: ";
	cin >> scredits;

	this->students[selected_student].set_id(sid);
	this->students[selected_student].set_name(sfullname);
	this->students[selected_student].set_semester(ssemester);
	this->students[selected_student].set_credits(scredits);
}
...
void Management::delete_student() {
	...
	this->students.erase(this->students.begin() + selected_student);
}
